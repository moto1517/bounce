...
void setup_ball3() {
	...
	speedY = 0;
	g = gravity;
	ball = color(44, 100, 100);
	table = color(147, 68, 29);
...
void ball3() {
	fill(ball);
	if (count > 0) {
		ellipse(width / 2 + diam * 0.01 * sin(random(0, 2 * PI)), y + diam * 0.01 * cos(random(0, 2 * PI)) + (diam / 2) * (1 - dis_y),((height - y - diam / 2) * diam_top + diam) * dis_x,((height - y - diam / 2) * diam_top + diam) * dis_y);
	} else {
		ellipse(width / 2, y,(height - y - diam / 2) * diam_top + diam,(height - y - diam / 2) * diam_top + diam);
	}
	y += speedY;
	speedY += g;
	dis_x = 1;
